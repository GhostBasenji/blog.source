+++
title = "Часть 1. Создаём проект и первый контроллер"
...
Цель серии — построить небольшой бэкенд для текстовой RPG: персонажи, боёвка, аутентификация, база данных. Небольшой, но достаточно реальный, чтобы разобраться со стеком.
...
Открываю VS Code. Встроенный терминал открывается через меню **Terminal → New Terminal** или сочетанием клавиш `` Ctrl+` ``.
...
Команда `dotnet new webapi` создаёт новый проект по шаблону Web API. После выполнения VS Code, скорее всего, предложит открыть папку — соглашаюсь. Также появится уведомление с предложением добавить файлы для отладки — тоже соглашаюсь, это создаст папку `.vscode` с нужными настройками.
...
Работает. Но обратите внимание: `"class": 1` — возвращается число, а не строка `"Knight"`. Это поведение по умолчанию. Исправить просто — добавляю в `Program.cs`:
